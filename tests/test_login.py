...
def test_login_wrong_method(client):
    ...
    # Then
    assert response.status_code == 405, f"Response status code: {response.status_code} and content: {content}"
    assert "detail" in content and content.get("detail") == "Método \"GET\" no permitido.", f"Error not found in response: {content}"
...
def test_login_empty_fields(client):
    ...
    assert "email" in content and content.get("email")[0] == "Este campo no puede estar en blanco.", f"Error not found in response: {content}"
    assert "password" in content and content.get("password")[0] == "Este campo no puede estar en blanco.", f"Error not found in response: {content}"
